...
FECHA_DE_CREACION:new Date()
  ...
  const data = await fetchPost(
    `${process.env.REACT_APP_SERVER}/api/partido_bloque/create`,
    this.state
  );
  this.setState({ data: data.data });
  alert(data.message);
  this.props.history.push(`/partido_bloques/${this.props.match.params.id}`);
};
  ...
  async componentDidMount() {
   ...
   const Equipos = await fetchGet(
    `${process.env.REACT_APP_SERVER}/api/partido_bloque/ByTorneoJuego/${id}`
  );
