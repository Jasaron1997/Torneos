...
FECHA_CREACION:new Date(this.state.FECHA_CREACION)
    ...
    const data = await fetchPost(
      `${process.env.REACT_APP_SERVER}/api/DETALLE_PARTIDO/Create_bloque`,
      this.state
    );
    this.setState({ data: data.data });
    alert(data.message);
    this.props.history.push(`/DETALLE_PARTIDO_bloque/${this.props.match.params.id}`);
  };
  async componentDidMount() {
    const Equipos = await fetchGet(
      `${process.env.REACT_APP_SERVER}/api/equipos/byPartidoBloque/${this.props.match.params.id}`
    );
