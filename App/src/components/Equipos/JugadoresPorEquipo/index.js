...
class Jugadores_Por_Equipo extends Component {
  constructor(props) {
    super(props);
  ...
  Buscar = async () =>{
    const data = await fetchGet(`${process.env.REACT_APP_SERVER}/api/jugadores_Por_Equipo/all`);
...
await this.setState({...item})
  ...
  const data = await fetchPost(
    `${process.env.REACT_APP_SERVER}/api/Jugadores_Por_Equipo/delete`,this.state
  );
  alert(data.message);
...
await this.setState({...estadoInicial})
  ...
  const dataGet = await fetchGet(
    `${process.env.REACT_APP_SERVER}/api/Jugadores_Por_Equipo/all`
  );
  ...
  render() {
    ...
    return (
      <Fragment>
        {redireccion}
        <h1 className="text-center mb-5">Jugadores_Por_Equipo</h1>
        ...
        {this.props.Access("1") && (
          <Link
            to={`${process.env.PUBLIC_URL}/Jugadores_Por_Equipo/crear`}
        ...
        {this.state.dataFiltrada && (
          <div className="ml-5 mr-5">
            <div className="row border">
              ...
              <div className="col-sm-3 col-xs-3">JUGADOR</div>
              <div className="col-sm-2 col-xs-2">POSICION</div>
            ...
            {this.state.dataFiltrada.map((item) => {
              ...
              return (
                <div className="row border" key={ID_EQUIPO}>
                  ...
                  <div className="col-sm-3 col-xs-3">{item.JUGADOR}</div>
                  <div className="col-sm-2 col-xs-2">{item.POSICION}</div>
                  <div className="col-sm-4 col-xs-4">
                
                    {this.props.Access("1")  && (
                      <Link
                        to={`${process.env.PUBLIC_URL}/Jugadores_Por_Equipo/modificar/${item.ID_JUGADOR_POR_EQUIPO}`}
                    ...
                    {this.props.Access("1") && (
                      <Link
                        to={`${process.env.PUBLIC_URL}/Jugadores_Por_Equipo/detalle/${item.ID_JUGADOR_POR_EQUIPO}`}
...
export default Jugadores_Por_Equipo;
