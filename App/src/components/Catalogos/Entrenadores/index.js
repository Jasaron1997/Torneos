...
class Entrenadores extends Component {
  ...
  Buscar = async () =>{
    const data = await fetchGet(`${process.env.REACT_APP_SERVER}/api/entrenadores/all`);
...
await this.setState({...item})
  ...
  const data = await fetchPost(
    `${process.env.REACT_APP_SERVER}/api/entrenadores/delete`,this.state
  );
  alert(data.message);
...
await this.setState({...estadoInicial})
  ...
  render() {
    ...
    return (
      <Fragment>
        {redireccion}
        <h1 className="text-center mb-5">Entrenadores</h1>
        ...
        {this.props.Access("1") && (
          <Link
            to={`${process.env.PUBLIC_URL}/entrenadores/crear`}
        ...
        {this.state.dataFiltrada && (
          <div className="ml-5 mr-5">
            ...
            {this.state.dataFiltrada.map((item) => {
              const { ID_ENTRENADOR } = item;
              return (
                <div className="row border" key={ID_ENTRENADOR}>
                  ...
                  <div className="col-sm-4 col-xs-4">
                    ...
                    {this.props.Access("1")  && (
                      <Link
                        to={`${process.env.PUBLIC_URL}/entrenadores/modificar/${item.ID_ENTRENADOR}`}
                    ...
                    {this.props.Access("1") && (
                      <Link
                        to={`${process.env.PUBLIC_URL}/entrenadores/detalle/${item.ID_ENTRENADOR}`}
